...
Particle::Particle(double ch, int z_1, int n_p)// initialization particle from input struct
{
	...
	n = n_p; // principal quantum number
	...
	dt = 0.0002668*pow(10, -14); //laser period for wavelength = 0.8 mkm or this must be the time step in PIC code? here we increase to -3 degree or else don't correctly work
}

void Particle::W_cal(double A, double B, double E, double n)//ionization rate ADK for DC
{
	W.insert(W.end(),A*pow(E, -(2 * n-1.5))*exp(-2* B / (3 *E))); // for vector massive
}

void Particle::W_cal_AC(double A, double B, double E, double n)//ionization rate ADK for AC
{
	W_AC.insert(W_AC.end(), sqrt((3*E)/(M_PI*B))*A*pow(E, -(2 * n - 1))*exp(-2 * B / (3 * E)));
}
...
void Particle::N_cal_dif() //see when ionizate from z_i=0 to k_max!!!!
{
	...
	for (int k = 1;k <= z1;k++) // while k (degree ionization at that step) < max charge calculate coef k [1;z1-z_i] (k_max-i)
		{
			...
			if (z1 == 1) { p += 1 - exp(-W.at(z1-1)*dt); }
				...
				{
					for (int h = 1;h <= z1 - (z_i + 1);h++) //sum for eq.(7), where P is the function in sum. h [1;k_max-i] and k!=k_max!=z1
					{
						p += P(k, h, z_i);
					};
				};
...
double Particle::P(int k_in, int p, int i)  //probability for i+k degree ionization for N_cal_dif (from eq.(7)) for N_cal_dif
{
	//double k = k_in - 1; // for save vaector's boundary 
	double m(0), res(1), prob_ion_k(0), prob_ion_p(0);
	prob_ion_k = W.at(k_in + i);
	prob_ion_p = W.at(p + i - 1);
	m = (exp(-prob_ion_k*dt) - exp(-prob_ion_p*dt)) / (1 - prob_ion_k / prob_ion_p);
	for (int j = 1;j <= k_in;j++)
		if (j != p) { res *= 1 - prob_ion_p / W.at(j + i - 1); };
	res = 1 / res;
	res *= m;
	return res;
}

double Particle::P_max(int k_in, int p, int i)
{
	double k = k_in - 1;
	double m(0), res(1), prob_ion_k(0), prob_ion_p(0);
	prob_ion_k = W.at(k);
	...
	for (int j = 1;j <= k-i;j++)
		if (j != p) { res *= 1 - prob_ion_p / W.at(j + i - 1); }; // difference between P and P_max
	res = 1 / res;
	res *= m;
	return res;
}
...
void Particle::P() // void because j - number of new electrons and degree of ionization reside in the object. DON'T FORGET set it zero after create new particle
{
	...
	{
		k++;
		if (k > z1) { exit = 1; } //condition to exit, when k=z1
		else
		{
			double u = urd(gen);
			sum1 = sum2;
			sum2 += N.at(k);
			if ((u > sum1) && (u < sum2)) { j = k; exit = 1; };
		}
	};
}